...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert a checkpoint file into a support sets and a reconstructor "
        "weights files"
    )
    parser.add_argument(
        "--exp",
    ...
    print(
        "#. Convert checkpoint file into support sets and reconstructor weight files..."
    )
    ...
    torch.save(
        checkpoint_dict["support_sets"],
        osp.join(models_dir, "support_sets-{}.pt".format(checkpoint_iter)),
    )

    # Save reconstructor weights file
    print("  \\__Save checkpoint reconstructor weights file...")
    torch.save(
        checkpoint_dict["reconstructor"],
        osp.join(models_dir, "reconstructor-{}.pt".format(checkpoint_iter)),
    )
