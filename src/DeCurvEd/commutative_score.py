...
def tensor2image(tensor, img_size=None, adaptive=False):
    ...
    if adaptive:
        ...
        if img_size:
            return ToPILImage()((255 * tensor.cpu().detach()).to(torch.uint8)).resize(
                (img_size, img_size)
            )
        else:
...
def build_gan(
    ...
    if gan_type == "BigGAN":
        G = build_biggan(
            pretrained_gan_weights=GAN_WEIGHTS[gan_type]["weights"][
                GAN_RESOLUTIONS[gan_type]
            ],
...
def manipulation(
    ...
    for _ in range(shift_steps):
        ...
        if cnt == args.shift_leap:
            ...
            current_path_latent_codes.append(
                w if args_json.__dict__["shift_in_w_space"] else z
            )
            cnt = 0
...
def manipulation_linear(
    ...
    if args_json.__dict__["flow"] == "FFJORD":
        with torch.no_grad():
            z_new = S.manipulate(
                w if args_json.__dict__["shift_in_w_space"] else z, support_sets_mask
            )
        if args_json.__dict__["shift_in_w_space"]:
            shift = z_new - w
            w = z_new
        ...
        support_sets_mask = torch.zeros(
            sum(shift_steps) + 1, args_json.__dict__["num_support_sets"]
        )
        ind = 0
        for i in range(len(shift_steps)):
            for j in range(shift_steps[i]):
...
def get_args():
    ...
    parser.add_argument(
        "--batch-size",
        type=int,
        help="set generator batch size (if not set, use the total number of "
        "images per path)",
    )
    parser.add_argument(
        "--img-size",
        type=int,
        help="set size of saved generated images (if not set, use the output "
        "size of the respective GAN generator)",
    )
    parser.add_argument(
        "--img-quality", type=int, default=95, help="set JPEG image quality"
    )
    ...
    parser.add_argument(
        "--dim", type=str, default="default", help="set translate dimensions (xx-xx)"
    )
    # ================================================================================================================ #
    parser.add_argument(
...
def main():
    ...
    models_dir_files = [
        f for f in os.listdir(models_dir) if osp.isfile(osp.join(models_dir, f))
    ]
    ...
    if not osp.isdir(pool):
        raise NotADirectoryError(
            "Invalid pool directory: {} -- Please run sample_gan.py to create it.".format(
                pool
            )
        )
    ...
    if args.verbose:
        ...
        print(
            "  \\__Pre-trained weights: {}".format(
                GAN_WEIGHTS[gan_type]["weights"][
                    args_json.__dict__["stylegan2_resolution"]
                ]
                if gan_type == "StyleGAN2"
                else GAN_WEIGHTS[gan_type]["weights"][GAN_RESOLUTIONS[gan_type]]
            )
        )
    ...
    if args_json.__dict__["flow"] == "FFJORD":
        ...
        S = SupportSets(
            ...
            gamma=1.0 / G.dim_z
            if args_json.__dict__["gamma"] is None
            else args_json.__dict__["gamma"],
        )
    ...
    S.load_state_dict(
        torch.load(support_sets_model, map_location=lambda storage, loc: storage)
    )
    ...
    latent_codes_dirs = [
        dI for dI in os.listdir(pool) if os.path.isdir(os.path.join(pool, dI))
    ]
    ...
    for subdir in latent_codes_dirs:
        latent_codes.append(
            torch.load(
                osp.join(pool, subdir, "latent_code.pt"),
    ...
    for i in range(num_of_latent_codes):
        ...
        if args.verbose:
            update_progress(
                "  \\__.Latent code hash: {} [{:03d}/{:03d}] ".format(
                    latent_code_hash, i + 1, num_of_latent_codes
                ),
                num_of_latent_codes,
                i,
            )
        ...
        transformed_images_root_dir = osp.join(
            latent_code_dir, "paths_images_eval_attr"
        )
        ...
        current_path_latent_codes = [
            G.get_w(z_) if args_json.__dict__["shift_in_w_space"] else z_
        ]
        current_path_latent_shifts = [
            torch.zeros_like(z_).cuda() if use_cuda else torch.zeros_like(z_)
        ]
        ...
        if args_json.__dict__["shift_in_w_space"]:
            ...
            z = z_.clone()
            w = None
        transformed_images_dir = osp.join(
            transformed_images_root_dir, "path_{}".format(dim[0])
        )
        ...
        for j in range(len(shift_steps)):
            img_path.append(
                osp.join(
                    ...
                    "{:06d}.jpg".format(sum(shift_steps[0 : j + 1])),
                )
            )

        if not os.path.isfile(img_path[-1]):
            if (
                args_json.__dict__["flow"] == "FFJORD"
                or args_json.__dict__["flow"] == "linear"
            ):
                current_path_latent_codes, current_path_latent_shifts = (
                    manipulation_linear(
                        use_cuda,
                        shift_steps,
                        dim,
                ...
                for j in range(len(attr_list)):
                    ...
                    w, z, current_path_latent_codes, current_path_latent_shifts = (
                        manipulation(
                            use_cuda,
                            shift_steps[j],
                            dim[j],
            ...
            current_path_latent_codes_batches = torch.split(
                current_path_latent_codes, args.batch_size
            )
            current_path_latent_shifts = torch.cat(current_path_latent_shifts)
            current_path_latent_shifts_batches = torch.split(
                current_path_latent_shifts, args.batch_size
            )

            if len(current_path_latent_codes_batches) != len(
                current_path_latent_shifts_batches
            ):
                ...
                with torch.no_grad():
                    if args_json.__dict__["shift_in_w_space"]:
                        transformed_img.append(
                            G(z=current_path_latent_codes_batches[t], latent_is_w=True)
                        )
                    else:
                        transformed_img.append(
                            G(z=current_path_latent_codes_batches[t])
                        )
        ...
        if "age" in attr_list:
            ...
            age_original_diffs.append(
                np.abs(age_predictions[age_index + 1] - age_predictions[age_index])
            )
        if "gender" in attr_list:
            gender_forward = femaleness_scores[-1]
            gender_index = attr_list.index("gender")
            gender_original_diffs.append(
                np.abs(
                    femaleness_scores[gender_index + 1]
                    - femaleness_scores[gender_index]
                )
            )
        if "race" in attr_list:
            race_forward = race_predictions[-1]
            race_index = attr_list.index("race")
            race_original_diffs.append(
                np.abs(race_predictions[race_index + 1] - race_predictions[race_index])
            )
        if "smile" in attr_list:
            smile_forward = celeba_attr["Smiling"][-1]
            smile_index = attr_list.index("smile")
            smile_original_diffs.append(
                np.abs(
                    celeba_attr["Smiling"][smile_index + 1]
                    - celeba_attr["Smiling"][smile_index]
                )
            )
        if "bangs" in attr_list:
            bangs_forward = celeba_attr["Bangs"][-1]
            bangs_index = attr_list.index("bangs")
            bangs_original_diffs.append(
                np.abs(
                    celeba_attr["Bangs"][bangs_index + 1]
                    - celeba_attr["Bangs"][bangs_index]
                )
            )
        if "beard" in attr_list:
            beard_forward = celeba_attr["No_Beard"][-1]
            beard_index = attr_list.index("beard")
            beard_original_diffs.append(
                np.abs(
                    celeba_attr["No_Beard"][beard_index + 1]
                    - celeba_attr["No_Beard"][beard_index]
                )
            )
        if "yaw" in attr_list:
            yaw_forward = yaw_predicted.cpu().numpy()[-1]
            yaw_index = attr_list.index("yaw")
            yaw_original_diffs.append(
                np.abs(
                    yaw_predicted.cpu().numpy()[yaw_index + 1]
                    - yaw_predicted.cpu().numpy()[yaw_index]
                )
            )
        if "roll" in attr_list:
            roll_forward = roll_predicted.cpu().numpy()[-1]
            roll_index = attr_list.index("roll")
            roll_original_diffs.append(
                np.abs(
                    roll_predicted.cpu().numpy()[roll_index + 1]
                    - roll_predicted.cpu().numpy()[roll_index]
                )
            )
        if "pitch" in attr_list:
            pitch_forward = pitch_predicted.cpu().numpy()[-1]
            pitch_index = attr_list.index("pitch")
            pitch_original_diffs.append(
                np.abs(
                    pitch_predicted.cpu().numpy()[pitch_index + 1]
                    - pitch_predicted.cpu().numpy()[pitch_index]
                )
            )
        ...
        if args_json.__dict__["shift_in_w_space"]:
            ...
            w = None
        transformed_images_dir = osp.join(
            transformed_images_root_dir, "path_{}".format(dim_reverse[0])
        )
        for d in range(1, len(dim_reverse)):
            transformed_images_dir = transformed_images_dir + "to{}".format(
                dim_reverse[d]
            )
        img_path = [osp.join(transformed_images_dir, "{:06d}.jpg".format(0))]
        for j in range(len(shift_steps_reverse)):
            img_path.append(
                osp.join(
                    transformed_images_dir,
                    "{:06d}.jpg".format(sum(shift_steps_reverse[0 : j + 1])),
                )
            )
        if not os.path.isfile(img_path[-1]):
            if (
        ...
        if "gender" in attr_list:
            faces_gender_diffs_pre.append(
                np.abs(femaleness_scores[-1] - gender_forward)
            )
        ...
        if "smile" in attr_list:
            faces_smile_diffs_pre.append(
                np.abs(celeba_attr["Smiling"][-1] - smile_forward)
            )
        ...
        if "bangs" in attr_list:
            faces_bangs_diffs_pre.append(
                np.abs(celeba_attr["Bangs"][-1] - bangs_forward)
            )
        ...
        if "beard" in attr_list:
            faces_beard_diffs_pre.append(
                np.abs(celeba_attr["No_Beard"][-1] - beard_forward)
            )
        ...
        if "yaw" in attr_list:
            faces_yaw_diffs_pre.append(
                np.abs(yaw_predicted.cpu().numpy()[-1] - yaw_forward)
            )
        ...
        if "roll" in attr_list:
            faces_roll_diffs_pre.append(
                np.abs(roll_predicted.cpu().numpy()[-1] - roll_forward)
            )
        ...
        if "pitch" in attr_list:
            faces_pitch_diffs_pre.append(
                np.abs(pitch_predicted.cpu().numpy()[-1] - pitch_forward)
            )
    ...
    with open(txt_out_dir, "w") as f:
        if "age" in attr_list:
            ...
            print(
                "age commutative score: {}, list is {}".format(
                    np.mean(faces_age_diffs_pre), faces_age_diffs_pre
                ),
                file=f,
            )
        if "gender" in attr_list:
            ...
            print(
                "gender commutative score: {}, list is {}".format(
                    np.mean(faces_gender_diffs_pre), faces_gender_diffs_pre
                ),
                file=f,
            )
        if "race" in attr_list:
            ...
            print(
                "race commutative score: {}, list is {}".format(
                    np.mean(faces_race_diffs_pre), faces_race_diffs_pre
                ),
                file=f,
            )
        if "smile" in attr_list:
            ...
            print(
                "smile commutative score: {}, list is {}".format(
                    np.mean(faces_smile_diffs_pre), faces_smile_diffs_pre
                ),
                file=f,
            )
        if "bangs" in attr_list:
            ...
            print(
                "bangs commutative score: {}, list is {}".format(
                    np.mean(faces_bangs_diffs_pre), faces_bangs_diffs_pre
                ),
                file=f,
            )
        if "beard" in attr_list:
            ...
            print(
                "beard commutative score: {}, list is {}".format(
                    np.mean(faces_beard_diffs_pre), faces_beard_diffs_pre
                ),
                file=f,
            )
        if "yaw" in attr_list:
            ...
            print(
                "yaw commutative score: {}, list is {}".format(
                    np.mean(faces_yaw_diffs_pre), faces_yaw_diffs_pre
                ),
                file=f,
            )
        if "roll" in attr_list:
            ...
            print(
                "roll commutative score: {}, list is {}".format(
                    np.mean(faces_roll_diffs_pre), faces_roll_diffs_pre
                ),
                file=f,
            )
        if "pitch" in attr_list:
            ...
            print(
                "pitch commutative score: {}, list is {}".format(
                    np.mean(faces_pitch_diffs_pre), faces_pitch_diffs_pre
                ),
                file=f,
            )
