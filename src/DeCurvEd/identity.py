...
def manipulation_linear(
    ...
    if args_json.__dict__["flow"] == "FFJORD":
        ...
        support_sets_mask = torch.zeros(
            shift_steps, args_json.__dict__["num_support_sets"]
        )
        for i in range(shift_steps):
            support_sets_mask[i, dim] = eps if signs == "+" else -eps
        if use_cuda:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="WarpedGANSpace latent space traversal script"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="set experiment's model dir (created by `train.py`)",
    )
    parser.add_argument(
        "--pool",
        type=str,
        required=True,
        help="directory of pre-defined pool of latent codes"
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="set number of shifts per positive/negative path " "direction",
    )
    parser.add_argument(
        "--eps", type=float, default=0.2, help="set shift step magnitude"
    )
    ...
    parser.add_argument(
        "--signs", type=str, default="plus", help="plus or minus direction"
    )
    # ================================================================================================================ #
    parser.add_argument(
    ...
    out_dir = osp.join(
        ...
        "{}_{}_{}_{}".format(
            args.shift_steps,
            args.eps,
            round(args.shift_steps * args.eps, 3),
            args.signs,
        ),
    )
    ...
    if args.verbose:
        ...
        print(
            "  \\__Traversal length : {}".format(round(args.shift_steps * args.eps, 3))
        )
    ...
    for i in range(num_of_latent_codes):
        ...
        if (
            ...
            if i < 50:
                ...
                signs = "-"
            current_path_latent_codes, current_path_latent_shifts = manipulation_linear(
                use_cuda,
                args.shift_steps,
                dim,
                G,
            ...
            w, z, current_path_latent_codes, current_path_latent_shifts = manipulation(
                use_cuda,
                args.shift_steps,
                dim,
                G,
        ...
        if args.attr is not None:
            ...
            for j in range(args.shift_steps):
                img_path.append(
                    osp.join(transformed_images_dir, "{:06d}.png".format(j + 1))
                )
            imgs = []
            ...
            faces_gender_diffs_pre.append(
                np.abs(femaleness_scores[-1] - femaleness_scores[0])
            )
            faces_race_diffs_pre.append(
                np.abs(race_predictions[-1] - race_predictions[0])
            )
            faces_smile_diffs_pre.append(
                np.abs(celeba_attr["Smiling"][-1] - celeba_attr["Smiling"][0])
            )
            faces_beard_diffs_pre.append(
                np.abs(celeba_attr["No_Beard"][-1] - celeba_attr["No_Beard"][0])
            )
            faces_bangs_diffs_pre.append(
                np.abs(celeba_attr["Bangs"][-1] - celeba_attr["Bangs"][0])
            )
            faces_yaw_diffs_pre.append(
                np.abs(yaw_predicted.cpu().numpy()[-1] - yaw_predicted.cpu().numpy()[0])
            )
            faces_pitch_diffs_pre.append(
                np.abs(
                    pitch_predicted.cpu().numpy()[-1] - pitch_predicted.cpu().numpy()[0]
                )
            )
            faces_roll_diffs_pre.append(
                np.abs(
                    roll_predicted.cpu().numpy()[-1] - roll_predicted.cpu().numpy()[0]
                )
            )
        ...
        torch.save(
            torch.cat(paths_latent_codes),
            osp.join(latent_code_dir, "paths_latent_codes.pt"),
        )
    ...
    txt_out_dir = out_dir + "/"
    txt_out_dir = (
        txt_out_dir + "age{}_".format(args.dim) if "age" in args.attr else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "gender{}_".format(args.dim)
        if "gender" in args.attr
        else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "race{}_".format(args.dim) if "race" in args.attr else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "smile{}_".format(args.dim)
        if "smile" in args.attr
        else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "bangs{}_".format(args.dim)
        if "bangs" in args.attr
        else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "beard{}_".format(args.dim)
        if "beard" in args.attr
        else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "roll{}_".format(args.dim) if "roll" in args.attr else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "pitch{}_".format(args.dim)
        if "pitch" in args.attr
        else txt_out_dir
    )
    txt_out_dir = (
        txt_out_dir + "yaw{}_".format(args.dim) if "yaw" in args.attr else txt_out_dir
    )
    txt_out_dir = txt_out_dir + ".txt"
    with open(txt_out_dir, "w") as f:
        print("indetity for {}".format(args.attr))
        print(
            "steps: {}, attr: {}, dim: {}".format(
                args.shift_steps, args.attr, args.dim
            ),
            file=f,
        )
        print("file path: {}".format(args.exp), file=f)
        print(
            "identity loss: {}, std: {}".format(
                np.mean(identity_losses), np.std(identity_losses)
            ),
            file=f,
        )
    ...
    with open(
        os.path.join(out_dir, "identity{}.pkl").format(args.shift_steps), "wb"
    ) as p:
