...
def sample_z(batch_size, dim_z, truncation=None):
    ...
    if truncation is None or truncation == 1.0:
        ...
        return torch.from_numpy(
            truncnorm.rvs(-truncation, truncation, size=(batch_size, dim_z))
        ).to(torch.float)
...
def create_exp_dir(args):
    ...
    exp_dir += "-{}".format(args.flow)
    exp_dir += "-reg{}".format(args.reg_flow)
    if args.small_dim:
        exp_dir += "-smalldim"
    exp_dir += "-seed{}".format(args.seed)
...
def update_progress(msg, total, progress):
    ...
    text = "\r{}{} {:.0f}% {}".format(
        msg,
        block_symbol * block + empty_symbol * (bar_length - block),
...
def sec2dhms(t):
    ...
    t %= 60
    seconds = t
    return "%02d days, %02d hours, %02d minutes, and %02d seconds" % (
        day,
        hour,
        minutes,
        seconds,
...
def create_summarizing_gif(
    ...
    path_images = [
        osp.join(imgs_root, dI)
        for dI in os.listdir(imgs_root)
        if osp.isfile(osp.join(imgs_root, dI))
    ]
    path_images.sort()
    ...
    for i in range(len(static_imgs)):
        static_img_pil.paste(
            Image.open(static_imgs[i]).resize((gif_w, gif_h)), (i * gif_w, 0)
        )
    ...
    for i in range(len(path_images)):
        ...
        gif_frame_pil = Image.new(
            "RGB", size=((num_imgs + 1) * gif_w + gap, gif_h), color=(255, 255, 255)
        )
        ...
        # Paste current image
        gif_frame_pil.paste(
            Image.open(path_images[i]).resize((gif_w, gif_h)),
            (num_imgs * gif_w + gap, 0),
        )
        ...
        if progress_bar_h > 0:
            ...
            gif_frame_pil_drawing.rectangle(
                xy=[
                    num_imgs * gif_w + gap,
                    gif_h - progress_bar_h,
                    num_imgs * gif_w + gap + progress,
                    gif_h,
                ],
