    ...
    def __init__(self, in_planes, out_planes, lightweight=False):
        ...
        if in_planes != out_planes:
            self.downsample = nn.Sequential(
                nn.Conv2d(
                    in_planes, out_planes, kernel_size=(1, 1), stride=(1, 1), bias=False
                ),
...
class HourGlass(nn.Module):
    ...
    def _generate_network(self, level):
        self.add_module(
            "b1_" + str(level),
            ConvBlock(self.features, self.features, lightweight=self.lightweight),
        )
...
class QFAN(nn.Module):
    def __init__(
        ...
        for hg_module in range(self.num_modules):
            ...
            self.add_module(
                "top_m_" + str(hg_module),
                ConvBlock(self.num_features, self.num_features),
            )
            self.add_module(
            ...
            self.add_module(
                "bn_end" + str(hg_module), nn.BatchNorm2d(self.num_features)
            )
            self.add_module(
                "l" + str(hg_module),
                nn.Conv2d(
...
class FANAU(nn.Module):
    def __init__(self, num_modules=1, num_features=128, n_points=66, block=ConvBlock):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(68, self.num_features, 1, 1),
            nn.BatchNorm2d(self.num_features),
            nn.ReLU6(),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(self.num_features, self.num_features, 1, 1),
            nn.BatchNorm2d(self.num_features),
            nn.ReLU6(),
        )
