...
class Trainer(object):
    def __init__(self, params=None, exp_dir=None, use_cuda=False, multi_gpu=False):
        if params is None:
            raise ValueError(
                "Cannot build a Trainer instance with empty params: params={}".format(
                    params
                )
            )
        else:
    ...
    def log_progress(self, iteration, mean_iter_time, elapsed_time, eta):
        ...
        print(
            "      \\__Classification loss : {:.08f}".format(
                stats["classification_loss"]
            )
        )
        print(
            "      \\__Regression loss     : {:.08f}".format(stats["regression_loss"])
        )
    ...
    def train(self, generator, support_sets, reconstructor):
        ...
        torch.save(
            support_sets.state_dict(), osp.join(self.models_dir, "support_sets_init.pt")
        )
        ...
        if self.use_cuda:
            ...
            support_sets.train()
            reconstructor.train()
        # Set requires_grad=False of generetor because its parameters are not updated.
        for param in generator.G.parameters():
            param.requires_grad = False

        # Set support sets optimizer
        support_sets_optim = torch.optim.Adam(
            support_sets.parameters(), lr=self.params.support_set_lr
        )

        # Set shift predictor optimizer
        reconstructor_optim = torch.optim.Adam(
            reconstructor.parameters(), lr=self.params.reconstructor_lr
        )
        ...
        if self.multi_gpu:
            print(
                "#. Parallelize G, R over {} GPUs...".format(torch.cuda.device_count())
            )
        ...
        if starting_iter == self.params.max_iter:
            print(
                "#. This experiment has already been completed and can be found @ {}".format(
                    self.wip_dir
                )
            )
        ...
        for iteration in range(starting_iter, self.params.max_iter + 1):
            ...
            if self.params.small_dim:
                target_support_sets_indices = torch.randint(
                    0, self.params.small_dim, [self.params.batch_size]
                )
            else:
                target_support_sets_indices = torch.randint(
                    0, self.params.num_support_sets, [self.params.batch_size]
                )
            if self.use_cuda:
            ...
            shift_magnitudes_pos = (
                self.params.min_shift_magnitude - self.params.max_shift_magnitude
            ) * torch.rand(
                target_support_sets_indices.size()
            ) + self.params.max_shift_magnitude
            shift_magnitudes_neg = (
                self.params.min_shift_magnitude - self.params.max_shift_magnitude
            ) * torch.rand(
                target_support_sets_indices.size()
            ) - self.params.min_shift_magnitude
            shift_magnitudes_pool = torch.cat(
                (shift_magnitudes_neg, shift_magnitudes_pos)
            )

            shift_magnitudes_ids = torch.arange(
                len(shift_magnitudes_pool), dtype=torch.float
            )
            target_shift_magnitudes = shift_magnitudes_pool[
                torch.multinomial(
                    input=shift_magnitudes_ids,
            ...
            support_sets_mask = torch.zeros(
                [self.params.batch_size, self.params.num_support_sets]
            )

            if self.use_cuda:
                support_sets_mask = support_sets_mask.cuda()
            for i, (index, val) in enumerate(
                zip(target_support_sets_indices, target_shift_magnitudes)
            ):
            ...
            if self.params.flow == "FFJORD":
                # z_new is in w space if self.params.shift_in_w_space is True.
                z_new, ldj = support_sets(
                    generator.get_w(z) if self.params.shift_in_w_space else z,
                    support_sets_mask,
            ...
            elif self.params.flow == "linear":
                shift = support_sets(
                    support_sets_mask * target_shift_magnitudes.reshape(-1, 1)
                )
            ...
            predicted_support_sets_indices, predicted_shift_magnitudes = reconstructor(
                img, img_shifted
            )

            # Calculate classification (cross entropy) and regression (mean absolute error) losses
            classification_loss = self.cross_entropy(
                predicted_support_sets_indices, target_support_sets_indices
            )
            regression_loss = torch.mean(
                torch.abs(predicted_shift_magnitudes - target_shift_magnitudes)
            )

            # Calculate total loss and back-propagate
            loss = (
                self.params.lambda_cls * classification_loss
                + self.params.lambda_reg * regression_loss
            )
            if self.params.flow == "FFJORD":
                loss = loss + self.params.reg_flow * ((ldj) ** 2).mean()
            loss.backward()
            ...
            self.stat_tracker.update(
                accuracy=torch.mean(
                    (
                        torch.argmax(predicted_support_sets_indices, dim=1)
                        == target_support_sets_indices
                    ).to(torch.float32)
                ).detach(),
            ...
            eta = elapsed_time * (
                (self.params.max_iter - iteration) / (iteration - starting_iter + 1)
            )
        ...
        torch.save(
            reconstructor.module.state_dict()
            if self.multi_gpu
            else reconstructor.state_dict(),
        ...
        for _ in range(10):
            print()
        print(
            "#.Training completed -- Total elapsed time: {}.".format(
                sec2dhms(elapsed_time)
            )
        )
