...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        return upfirdn2d(x, self.kernel, up=1, down=self.factor, pad=self.pad)
    ...
    def forward(self, x):
        if self.activation:
            return fused_leaky_relu(
                F.linear(x, self.weight * self.scale), self.bias * self.lr_mul
            )
        else:
...
class ScaledLeakyReLU(nn.Module):
    ...
    def forward(self, x):
        return math.sqrt(2) * F.leaky_relu(x, negative_slope=self.negative_slope)
...
class ConstantInput(nn.Module):
    ...
    def forward(self, x):
        return self.input.repeat(x.shape[0], 1, 1, 1)
...
class StyledConv(nn.Module):
    ...
    def forward(self, x, style, noise=None):
        return self.activate(self.noise(self.conv(x, style), noise=noise))
...
class ToRGB(nn.Module):
    ...
    def forward(self, x, style, skip=None):
        if skip is not None:
            return self.conv(x, style) + self.bias + self.upsample(skip)
        else:
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        return (self.conv2(self.conv1(x)) + self.skip(x)) / math.sqrt(2)
