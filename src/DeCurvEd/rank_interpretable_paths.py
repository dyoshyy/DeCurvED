...
def save_results(
    ...
    attr_idx_corr_df.to_csv(
        path_or_buf=osp.join(out_dir, "attr_idx_{}.csv".format(metric)),
        header=attributes,
        index_label="path_id",
        float_format="%.3f",
    )
    ...
    for t in range(attr_idx_corr_df.shape[1]):
        ...
        attr_idx_corr_df_sorted_by_t = attr_idx_corr_df.sort_values(
            by=t, ascending=False
        )
        first_rows.append(attr_idx_corr_df_sorted_by_t.to_numpy()[0, :])

        # Get ranked interpretable paths
        interpretable_paths_dict[metric][attributes[t]] = (
            attr_idx_corr_df_sorted_by_t.index.tolist()
        )
        ...
        attr_idx_corr_df_sorted_by_t.to_csv(
            path_or_buf=osp.join(
                out_dir, "attr_idx_{}_sorted_by_{}.csv".format(metric, attributes[t])
            ),
            header=attributes,
            float_format="%.3f",
        )
    ...
    pd.DataFrame(np.stack(first_rows)).to_csv(
        path_or_buf=osp.join(out_dir, "attr_idx_{}_diag.csv".format(metric)),
        header=attributes,
        float_format="%.2f",
    )
            ...
            for k in range(top_k):
                for h in hashes:
                    ...
                    imgs_root = osp.join(
                        ...
                        h,
                        "paths_images",
                        "path_{:03d}".format(top_k_paths[k][a]),
                    )
                    gif_filename = osp.join(
                        attr_dir,
                        "{}_{}_{}_{}.gif".format(attr, k + 1, top_k_paths[k][a], h),
                    )
...
def create_summary_md_file(
    ...
    md_summary_file = osp.join(
        interpretable_paths_root,
        "top-{}_interpretable_path_{}.md".format(top_k, attr_group),
    )
        ...
        for h in hashes:
            ...
            for m in metrics:
                ...
                for k in range(top_k):
                    ...
                    gif_file = osp.join(
                        m, attr, "{}_{}_{}_{}.gif".format(attr, k + 1, path_id, h)
                    )
                    gif_mouseover = (
                        "top-{} interpretable path [path_id: {}] for {}".format(
                            k + 1, path_id, attr
                        )
                    )
                    f.write(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="WarpedGANSpace interpretable path ranking script"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gif", dest="gif", action="store_true", help="create summarizing GIF files"
    )
    parser.add_argument(
        "--no-gif",
        dest="gif",
        action="store_false",
        help="do NOT create summarizing GIF files",
    )
    parser.set_defaults(gif=True)
    parser.add_argument(
        "--num-imgs",
        type=int,
        help="set number of static images per sequence; by default all "
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="create summarizing GIFs for the top-k interpretable path "
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=ATTRIBUTE_GROUPS.keys(),
        help="set attribute group -- see/edit `ATTRIBUTE_GROUPS` dictionary above",
    )
    parser.add_argument(
        "--metric",
        type=str,
        default="corr+corr_l1",
        choices=("corr", "corr_l1", "corr+corr_l1"),
    ...
    for l_config in latent_space_traversal_configs:
        ...
        hashes = [
            dI
            for dI in os.listdir(hashes_root)
            if osp.isdir(os.path.join(hashes_root, dI))
            and dI not in ("paths_gifs", "interpretable_paths")
        ]
        ...
        for i in range(len(hashes)):
            ...
            for a in attributes:
                ...
                try:
                    ...
                    print(
                        "Attribute traversal file not found: {}".format(
                            attr_traversal_file
                        )
                    )
        ...
        interpretable_paths_root = osp.join(
            hashes_root, "interpretable_paths", "Group_{}".format(args.attr_group)
        )
        ...
        if args.verbose:
            print(
                "           \\__.Attributes matrix (ATTRIBUTES) : {}".format(
                    ATTRIBUTES.shape
                )
            )
            print(
                "               \\__Number of samples           : {}".format(
                    num_of_samples
                )
            )
            print(
                "               \\__Number of paths             : {}".format(
                    num_of_paths
                )
            )
            print(
                "               \\__Number of attributes        : {}".format(
                    num_of_attributes
                )
            )
            print(
                "               \\__Number of points per path   : {}".format(
                    num_of_points_per_path
                )
            )

        with open(
            osp.join(interpretable_paths_root, "attributes_traversals_details.json"),
            "w",
        ) as fp:
            json.dump(
                {
        ...
        with open(
            osp.join(hashes_root, "interpretable_paths", "attributes_groups.json"), "w"
        ) as fp:
            json.dump(ATTRIBUTE_GROUPS, fp)

        # Calculate attribute-to-path correlations
        ATTRIBUTES_IDX_CORR = np.zeros(
            (num_of_samples, num_of_paths, num_of_attributes)
        )
        for s in range(num_of_samples):
            for k in range(num_of_paths):
                ...
                A_scaled = (
                    2.0
                    * (A.transpose() - attr_ranges[:, 0])
                    / (attr_ranges[:, 1] - attr_ranges[:, 0])
                    - 1.0
                ).transpose()
                ...
                for t in range(A.shape[0]):
                    ...
                    if "identity" in attributes:
                        if t == attributes.index("identity"):
                            A_t_idx = np.concatenate(
                                [
                                    -1 * np.arange(-A_t.shape[0] // 2 + 1, 0),
                                    np.arange(A_t.shape[0] // 2 + 1),
                                ]
                            )
                    ATTRIBUTES_IDX_CORR[s, k, t] = np.cov(A_t, A_t_idx)[0, 1] / np.sqrt(
                        np.cov(A_t_idx)
                    )
        ...
        create_summary_md_file(
            attr_group=args.attr_group,
            summary_md_dict=summary_md_dict,
            metric=args.metric,
            top_k=args.top_k,
            hashes=hashes,
        ...
        with open(
            osp.join(interpretable_paths_root, "interpretable_paths.json"), "w"
        ) as fp:
