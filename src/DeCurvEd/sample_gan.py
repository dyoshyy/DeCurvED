...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sample a pre-trained GAN latent space and generate images"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-samples", type=int, default=4, help="number of latent codes to sample"
    )
    ...
    if args.gan_type == "BigGAN":
        ...
        if args.biggan_target_classes is None:
            raise parser.error(
                "In case of BigGAN, a list of classes needs to be determined."
            )
        biggan_classes = ""
    ...
    if args.verbose:
        ...
        print(
            "  \\__Pre-trained weights: {}".format(
                ...
                else GAN_WEIGHTS[args.gan_type]["weights"][
    ...
    elif args.gan_type == "StyleGAN2_car":
        G = build_stylegan2(
            resolution=args.stylegan2_resolution,
            pretrained_gan_weights=GAN_WEIGHTS[args.gan_type]["weights"][
                args.stylegan2_resolution
    ...
    elif args.gan_type == "StyleGAN2_mpi3d":
        G = build_stylegan2(
            resolution=args.stylegan2_resolution,
            pretrained_gan_weights=GAN_WEIGHTS[args.gan_type]["weights"][
                args.stylegan2_resolution
    ...
    elif args.gan_type == "StyleGAN2_scene":
        G = build_stylegan2(
            resolution=args.stylegan2_resolution,
            pretrained_gan_weights=GAN_WEIGHTS[args.gan_type]["weights"][
                args.stylegan2_resolution
    ...
    elif args.gan_type == "StyleGAN2_isaac3d":
        G = build_stylegan2(
            resolution=args.stylegan2_resolution,
            pretrained_gan_weights=GAN_WEIGHTS[args.gan_type]["weights"][
                args.stylegan2_resolution
    ...
    elif args.gan_type == "StyleGAN2_cub":
        G = build_stylegan2(
            resolution=args.stylegan2_resolution,
            pretrained_gan_weights=GAN_WEIGHTS[args.gan_type]["weights"][
                args.stylegan2_resolution
    ...
    if args.verbose:
        print(
            "#. Sample {} {}-dimensional latent codes...".format(
                args.num_samples, G.dim_z
            )
        )
        if args.z_truncation:
            print(
                "  \\__Truncate standard Gaussian to range [{}, +{}]".format(
                    -args.z_truncation, args.z_truncation
                )
            )

    # zs = torch.randn(args.num_samples, G.dim_z)
    zs = sample_z(
        batch_size=args.num_samples, dim_z=G.dim_z, truncation=args.z_truncation
    )
    ...
    for i in range(args.num_samples):
        ...
        if args.verbose:
            update_progress(
                "  \\__.Latent code hash: {} [{:03d}/{:03d}] ".format(
                    latent_code_hash, i + 1, args.num_samples
                ),
                args.num_samples,
                i,
            )
        ...
        img_pil.save(
            osp.join(latent_code_dir, "image.jpg"),
            "JPEG",
            quality=95,
            optimize=True,
            progressive=True,
