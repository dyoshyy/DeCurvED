...
def main():
    ...
    parser.add_argument(
        "--gan-type",
        type=str,
        choices=GAN_WEIGHTS.keys(),
        help="set GAN generator model type",
    )
    parser.add_argument(
        "--z-truncation",
        type=float,
    ...
    parser.add_argument(
        "--biggan-target-classes",
        nargs="+",
        type=int,
        help="list of classes for conditional BigGAN",
    )
    parser.add_argument(
        "--stylegan2-resolution",
        type=int,
        default=1024,
    ...
    parser.add_argument(
        ...
        "--num-support-sets",
        type=int,
        help="set number of support sets (warping functions)",
    )
    parser.add_argument(
        "-D",
        "--num-support-dipoles",
        type=int,
        help="set number of support dipoles per support set",
    )
    parser.add_argument(
        "--learn-alphas", action="store_true", help="learn RBF alpha params"
    )
    parser.add_argument(
        "--learn-gammas", action="store_true", help="learn RBF gamma params"
    )
    parser.add_argument(
        "-g",
        "--gamma",
        type=float,
        help="set RBF gamma param; when --learn-gammas is set, this will "
        "be the initial value of gammas for all RBFs",
    )
    parser.add_argument(
        "--support-set-lr", type=float, default=1e-4, help="set learning rate"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="set minimum shift magnitude",
    )
    parser.add_argument(
        "--max-shift-magnitude",
        type=float,
        default=0.45,
        help="set shifts magnitude scale",
    )
    parser.add_argument(
        "--reconstructor-lr",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default="FFJORD",
        help="nice, realnvp, glow, FFJORD, RBF, or linear",
    )
    parser.add_argument(
        "--reg-flow", type=float, default=1.0, help="regularizer loss weight"
    )
    parser.add_argument(
        "--small-dim", action="store_true", help="edit part of dimensions"
    )
    parser.add_argument(
        "--flow-modules",
        type=str,
        default="512-512-512-512-512",
        help="flow modules for FFJORD",
    )
    ...
    parser.add_argument(
        "--lambda-cls", type=float, default=1.00, help="classification loss weight"
    )
    parser.add_argument(
        "--lambda-reg", type=float, default=0.25, help="regression loss weight"
    )
    parser.add_argument(
        "--log-freq", default=10, type=int, help="set number iterations per log"
    )
    parser.add_argument(
        "--ckp-freq",
        default=1000,
    ...
    if args.gan_type == "StyleGAN2":
        print(
            "  \\__Search for paths in {}-space".format(
                "W" if args.shift_in_w_space else "Z"
            )
        )
    ...
    print(
        "  \\__Pre-trained weights: {}".format(
            GAN_WEIGHTS[args.gan_type]["weights"][args.stylegan2_resolution]
            if args.gan_type == "StyleGAN2"
    ...
    if not args.learn_gammas:
        print(
            "  \\__RBF gamma                 : {}".format(
                1.0 / G.dim_z if args.gamma is None else args.gamma
            )
        )
